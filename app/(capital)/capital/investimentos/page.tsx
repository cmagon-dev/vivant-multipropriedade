...
export default async function CapitalInvestimentosPage() {
  ...
  const participations = await prisma.capitalParticipation.findMany({
    ...
    include: {
      assetConfig: {
        include: {
          property: { select: { id: true, name: true, slug: true, location: true } },
        },
      },
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-vivant-navy">Meus Investimentos</h1>
        <p className="text-gray-500 mt-1">Lista dos ativos em que você possui participação</p>
      </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-500">
                        ...
                        <span>{fmt(Number(p.valorAportado))} investido</span>
                        <span>{fmt(Number(p.expectedReturn ?? 0))} retorno estimado</span>
                        <span>{Number(p.roiPercent ?? 0).toFixed(2)}% ROI</span>
                        <span className="px-2 py-0.5 rounded bg-slate-100">{p.status}</span>
                      </div>
                    </div>
                  </div>
                  <Link
                    href={`/capital/ativos/${p.assetConfigId}`}
